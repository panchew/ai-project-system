...
def test_validation_warnings_separated_correctly(registry):
    """Test that warnings are properly separated between enrolled fleet projects and 
    all configs that were checked."""
    validation = registry["validation"]
    enumeration = registry["enumeration"]
    
    # Verify that we now have distinct fields for fleet warnings vs raw total
    assert "fleet_warnings" in validation
    assert "raw_total_warnings" in validation
    
    # Check that the relationship holds: raw_total_warnings >= fleet_warnings
    assert validation["raw_total_warnings"] >= validation["fleet_warnings"]
    
    # Verify that we have correct counts of configs checked and excluded 
    assert enumeration["fleet_configs_checked"] == 13
    assert enumeration["excluded_configs"] == 2
    
    # The diff between raw_total_warnings and fleet_warnings should match excluded contributions
    # But since we don't have explicit data on this, just validate field presence and relation
    assert isinstance(validation["fleet_warnings"], int)
    assert isinstance(validation["raw_total_warnings"], int)
