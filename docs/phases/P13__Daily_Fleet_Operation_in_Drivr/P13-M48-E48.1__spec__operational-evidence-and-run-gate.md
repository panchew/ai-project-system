...
**Must include:**
- A committed **input adapter** (a named command, e.g. `bin/drivr-events-to-instrument <record.json>`)
  that maps Drivr structured events onto the instrument's transcript document: each executed
  `part.tool`/tool-call event becomes a `{tool_call: {name, args}, tool_result}` entry with
  name/args/final-answer resolved from the event parts; the final message becomes `final_answer`.
- **Tests** for the adapter against the real `P12-M47-E47.3` dispatch-record shape (complete,
  partial, and missing event fields), asserting the instrument can then load the adapted document
  and produce counts.
- An explicit statement of **what does not map** (e.g. Drivr events carry `snapshot`/`reason`
  rather than a local-runner `tool_result` body; the adapter records denials/refusals only where the
  event actually carries them, and never fabricates a `tool_result`).
...
**Commit mechanism and cross-repository enforcement (concrete):**

- The instantiated `quality-bar.yml` — carrying both the floor and the quoted acceptance criteria —
  is committed to the **target project's dispatch branch** (not to `ai-project-system`), so the bar
  and the run live in the same git graph and the bar is fixed before dispatch in that graph. The M48
  record (E48.4/E48.5) references that commit.
- **Cross-repository pre-dispatch proof:** git ancestry cannot order commits across
  `ai-project-system` and the target project's graphs. The proof that the bar preceded dispatch is
  therefore a **recorded pair of resolved refs + commit timestamps**, committed to the run record:
  `bar_commit` (target-project repo, branch, SHA, author date) and `dispatch_commit` (same repo,
  SHA, author date), with the requirement `bar_commit.author_date <= dispatch_commit.author_date`
  verified at record time. A run record missing the paired refs, or with a bar dated after dispatch,
  is **fail-closed**.

E48.1 does **not** select a project Epic — that is E48.4/E48.5's own hierarchy. What E48.1 commits
here is the **two-layer bar (activity floor + criteria-and-review requirement), the quote-by-path
rule, and the cross-repository pre-dispatch proof**, so the moment a project Epic is selected, its
bar is a fill-in of an already-fixed contract with both thresholds already set — not a new design.
...
- **Quarantine location — outside Git.** A quarantined raw artifact is moved (not deleted) to a
  fixed host path **outside every repository**, e.g. `~/soft-dev/.m48-quarantine/<run>/`, which is
  never staged, tracked, or swept into any commit. Nothing secret-bearing is written under
  `.ai-project/` or any git tree. The commit carries **only** the redacted notice/reference (below);
  the secret-bearing source never enters any repository.
- **Safe reference format.** The record cites a quarantined source **by reference only** —
  `{"source": "~/soft-dev/.m48-quarantine/<run>/<file>", "redacted": true, "regions": [<offsets>]}`
  — with the matched regions redacted; the secret-bearing content is **never** embedded in any
  committed artifact, not even in the notice. The committed artifact is a redacted notice, never the
  raw source.
...
- [ ] **D3** — The two-layer quality-bar contract: the activity floor (`epic_dev` rounds>0 AND
      files>0) **plus** the selected Epic's acceptance criteria and independent-review disposition;
      cross-repository pre-dispatch proof (paired refs + timestamps), committed
...
- [ ] The cross-repository pre-dispatch proof (paired `bar_commit`/`dispatch_commit` refs +
      timestamps, bar not after dispatch) is specified and is the documented enforcement, not git
      ancestry across graphs
...
**Visual binding**
- **Link:** https://github.com/panchew/ai-project-system/blob/milestone/M48/docs/phases/P13__Daily_Fleet_Operation_in_Drivr/P13-M48-E48.1__spec__operational-evidence-and-run-gate.md
- **What:** diagram
- **Level:** Epic
- **State:** proposed
...
flowchart LR
  ...
  ADAPTER["D2a INPUT ADAPTER<br/>Drivr raw events → instrument transcript<br/>tested against P12-M47-E47.3 shape"]
  BAR["D3 TWO-LAYER BAR<br/>activity floor + Epic acceptance criteria<br/>+ independent review + cross-repo proof"]
  INVOKE["D4 INVOCATION PROCEDURE<br/>OpenCodeAdapter.execute + HostEnvironment<br/>presence-only checks + labeled probe"]
  SCAN["D4a SECRET-SCAN GATE<br/>bin/retain-raw-events — quarantine OUTSIDE git<br/>safe redacted notice, fail-closed"]
...
- **Description:** E48.1 commits the common pre-run contract before any project result is visible —
  ...
  normalization path over raw OpenCode events plus a tested input adapter that maps Drivr raw events
  onto the instrument's transcript document, a two-layer quality bar (activity floor *plus* the
  selected Epic's acceptance criteria with an independent-review disposition, and a cross-repository
  pre-dispatch proof), an adapter-level invocation procedure with presence-only checks and a labeled
  probe, an executable secret-scan gate that quarantines **outside Git** and commits only a redacted
  notice, the `panchew-io` baseline re-measure, and the per-target eligibility authority record.
  Proposed-track Structural diagram (Mermaid); `Link` is the hosted GitHub permalink to the
  committed diagram.
