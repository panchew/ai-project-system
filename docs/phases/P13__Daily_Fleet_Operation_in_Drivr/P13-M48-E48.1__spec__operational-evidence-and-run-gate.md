...
| `acceptance_criteria` | **the selected Epic's own acceptance criteria, quoted by path and branch**, each path + branch resolving at the immutable `target_commit_sha` (below) |
| `target_commit_sha` | **the 40-character immutable target-repository commit SHA** at which the quoted acceptance-criteria path and branch resolve — the required immutable identity (per the resolved escalation) |
...
2. **Task-specific quality acceptance (the actual bar):** the selected Epic's **committed acceptance
   criteria** (quoted by path and branch, resolving at the immutable `target_commit_sha`), judged
   **pass/fail by an independent project-level reviewer** — who is not the run's own report, not the
   executor, and not the Epic Chat that collected the evidence. The reviewer's disposition
   (pass/fail + defects + rework + intervention) is the quality verdict; the activity floor alone
   never produces a PASS.
...
- **Quality-bar ownership stays in `ai-project-system`, but the instantiation is E48.4/E48.5's.** E48.1
  specifies the **reusable bar schema** (including the required `target_commit_sha` and the quote-by-
  path-and-branch rule) — it does **not** own the instantiated artifacts. The instantiated
  E48.4/E48.5 `quality-bar.yml` files are **M48 evidence artifacts** under
  `.ai-project/artifacts/agentic-runs/P13-M48-<E>/`, committed on their own `epic/E48.4`/`epic/E48.5`
  branches (not on `epic/E48.1`). Each cites the selected project's Epic spec and acceptance criteria
  by **target-repository path, branch, and immutable commit SHA** (`target_commit_sha`). No bar is
  written into a fleet project, and none routes a write-through its governance chain.
- **Pre-dispatch ordering is proved in the M48 evidence graph + runtime record.** Before invoking
  Drivr, E48.4/E48.5 commit a **dispatch manifest** (bar commit SHA + target-project worktree HEAD)
  on their own Epic branches; the **bar commit must be an ancestor of the manifest commit** (same-graph
  ancestry). The invocation records that manifest SHA and Drivr's runtime `dispatched_at` — or, for
  the direct P12 adapter path (no scheduler), the supervisor's UTC `started_at` captured immediately
  before `OpenCodeAdapter.execute`. Missing ancestry, manifest identity, target HEAD, or runtime start
  time is **fail-closed**. **Git author dates are never ordering evidence.**

E48.1 does **not** select a project Epic and does **not** instantiate E48.4/E48.5's bars or manifests
— those are E48.4/E48.5's own hierarchy and branches. What E48.1 commits here is the **contract
only**: the reusable bar schema (two-layer: activity floor + criteria-and-review requirement, quote
by immutable-SHA path), the manifest schema, and the enforcement rules — so the moment a project Epic
is selected, its bar is a fill-in of an already-fixed contract, on the owning Epic's branch, not a
redesign.
...
**Repository:** all E48.1 artifacts — the contract, normalization code and tests, quality-bar
**schema** (incl. `target_commit_sha`), manifest **schema**, input adapter, invocation procedure,
secret-scan gate, re-measure record, eligibility record — and the Delivery Notice land in
`ai-project-system` on `epic/E48.1`. The **instantiated** E48.4/E48.5 `quality-bar.yml` files and
dispatch manifests are **not** E48.1 artifacts; they belong to `epic/E48.4`/`epic/E48.5` per the
Phase Ruling. The only cross-repository reads are `panchew-io` (model block, ref only), Drivr
(`OpenCodeAdapter` source, ref only), the two benched projects' registry state, and the selected
project's Epic spec + acceptance criteria (cited by immutable `target_commit_sha`) — **no write to
...
| Version | Date | Change |
|---|---|---|
| 1.5.0 | 2026-09-08 | **Replacement `+1` — applies the final escalation resolution (findings 2–4 only), on the synchronized branch (ruling `125bf58` now an ancestor via merge `ff7a320`).** (2) E48.1 is contract-only again: it specifies the reusable bar schema, manifest schema, adapters, tests, and procedure, but the **instantiated** E48.4/E48.5 `quality-bar.yml` files and dispatch manifests belong to `epic/E48.4`/`epic/E48.5`, not `epic/E48.1` (fixed in §3 commit-mechanism and §Technical Constraints). (3) The bar schema now carries the required **`target_commit_sha`** field (40-character target-repository commit SHA), and the quoted acceptance-criteria path+branch must resolve at that SHA. (4) Unknown tool names are valid executed rounds for C-A and are not relabeled mutating; only **missing or malformed tool/result/`files_changed` evidence** yields `undetermined`/`ERROR` (fixed in the Starter where the two clauses had been conflated). |
