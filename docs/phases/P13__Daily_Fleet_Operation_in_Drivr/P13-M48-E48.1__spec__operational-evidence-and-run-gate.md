...
**Visual binding**
- **Link:** deferred to implementation — no hosted permalink exists at planning time, and the M48
  Milestone planning chat does not mint hosted URLs. The Epics Chat records the hosted `Link` in
  this Epic's Delivery Notice `Visual Bindings` section when it hosts the implemented diagram
  (visual-artifacts.md §7: `Link` required, never a committed or local path; `visual_required_for_specs`
  is enforced there, not at planning).
- **What:** diagram
- **Level:** Epic
- **State:** proposed
...
flowchart LR
  ...
  BAR["D3 QUALITY-BAR CONTRACT<br/>instrument + concrete floor (rounds&gt;0 AND files&gt;0)<br/>committed before dispatch, fail-closed"]
  INVOKE["D4 INVOCATION PROCEDURE<br/>OpenCodeAdapter.execute + HostEnvironment<br/>presence-only checks + labeled probe"]
  SCAN["D4a SECRET-SCAN GATE<br/>bin/retain-raw-events — executable,<br/>quarantine + safe reference, fail-closed"]
  ...
  CONTRACT --> NORM --> BAR --> INVOKE --> SCAN --> BASELINE --> ELIG

  classDef a fill:#1e3a5f,stroke:#93c5fd,color:#fff
  class CONTRACT,NORM,BAR,INVOKE,SCAN,BASELINE,ELIG a
```
...
- **Description:** E48.1 commits the common pre-run contract before any project result is visible —
  ...
  normalization path over raw OpenCode events, a quality-bar contract carrying the concrete
  `epic_dev` floor (rounds > 0 AND files changed > 0) committed before dispatch, an adapter-level
  invocation procedure with presence-only checks and a labeled probe, an executable secret-scan gate
  (`bin/retain-raw-events`), the `panchew-io` baseline re-measure, and the per-target eligibility
  authority record. Proposed-track Structural diagram (Mermaid); `Link` is deferred to the
  implemented binding in the Delivery Notice.
