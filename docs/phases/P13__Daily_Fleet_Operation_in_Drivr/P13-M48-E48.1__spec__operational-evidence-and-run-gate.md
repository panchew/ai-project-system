...
### 3. Task-specific quality bars — a real contract, not a placeholder (D3)

Produce the **quality-bar contract now, fully specified so E48.4/E48.5 instantiate rather than
invent it**. The bar is designed before the result is seen, never after.

**Must include:**
- The **bar schema**, fixed field-for-field: bar ID + governing task identity (project, selected
  Epic, selected spec/Starter ref), the acceptance instrument to run (the reusable
  `bin/successful-nothing-instrument`-style measure, by name and interface), the per-lane pass floor
  (rounds, files changed, claims-resolution), the review-result fields to record against it
  (defects, rework count, intervention), and the missing-data fields with reasons.
- A **concrete mechanism** by which the selected project Epic's instantiating bar is **committed to
  the run record before dispatch** — a committed `quality-bar.yml`-style entry at a fixed path under
  the E48.4/E48.5 record, referenced by the run record D1 schema. Not prose-in-a-commit-message, not
  a "to be determined."
- The rule that the bar commit **precedes** the dispatch commit, verified by commit ordering, and
  that a run record lacking a committed bar is **fail-closed** (cannot be reported delivery-ready).

E48.1 does **not** select a project Epic — that is E48.4/E48.5's own hierarchy. What it delivers
here is the entire reusable structure and the enforcement rule, so the moment a project Epic is
selected, its bar is a fill-in of an already-fixed contract, not a new design.
...
- Raw-event retention instructions (transcript + dispatch record kept verbatim), **gated by a
  fail-closed secret scan** before any raw artifact is committed: every retained raw event set is
  passed through a committed detector (pattern + context scan mirroring the M49 secret-boundary
  work, in its pre-M49 minimal form), and any match **quarantines** the raw artifact rather than
  committing it verbatim. A quarantined raw source is retained by reference with the matched regions
  redacted, never by embedding the secret-bearing content. The scan result (clean / quarantined) is
  recorded in the record itself; **a missed scan is a fail-closed gap, not a warning**.
...
- [ ] **D3** — The quality-bar contract (bar schema + committed-bar mechanism + pre-dispatch ordering
      + fail-closed rule), committed
- [ ] **D4** — The adapter-level invocation procedure with presence-only route/credential checks and
      the fail-closed secret-scan/quarantine gate on raw-event retention
...
- [ ] The run-record contract and quality-bar contract are committed, and the contract distinguishes
...
- [ ] The raw-event retention path runs a fail-closed secret scan before committing any raw artifact;
      any match quarantines the artifact and redacts the matched regions (mirrored M49 pattern), and
      a missed scan is a fail-closed gap
