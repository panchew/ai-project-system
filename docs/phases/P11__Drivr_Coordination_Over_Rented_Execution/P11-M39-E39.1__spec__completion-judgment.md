...
| `final_answer` | unexecuted **tool-call JSON** | prose claiming success — correct *here* |
...
> **⚠ Amended at spec v1.0.1 — `final_answer` is ALSO wrong in both directions, and it is no longer
> described as a tell.** The milestone spec (v1.0.1) and this spec's v1.0.0 both marked `final_answer`
> as *"✅ a tell"* on Run A. **Falsified by F8 below**, from three cases preserved in this repository:
> `P7-M26-E26.3-PROVE`'s `final_answer` is the flat refusal *"I'm sorry, but I can't assist with that
> request."* on a run whose **ground truth is completed**, and `P9-M31-E31.1-PROVE`'s reports blocked
> permissions on a run that **converged on attempt 1 of 3**. **`final_answer` joins the exit code and
> `status` on the ruled-out list.**
...
### F8 — The preserved corpus is **six** runs, not two, and it falsifies `final_answer`

*(Added at spec v1.0.1. Measured 2026-08-15 on `milestone/M39`; every ground truth below is read from
a committed governance record, not inferred from the transcript.)*

`.ai-project/artifacts/agentic-runs/` holds **six schema-identical runs**, all git-tracked, all
carrying `status` / `final_answer` / `transcript` / `iterations` / `tokens` / `model` /
`duration_ms`. The milestone spec inventories two of them. **Every inventory is a floor.**

| Case | `status` | dev exit | iters | `final_answer` reads | **Ground truth** | Truth source |
|---|---|---|---|---|---|---|
| **E33.2 Run A** | `completed` | 0 | 0 | success-shaped tool-call JSON | **did not complete** | `run-record.md` §4 |
| E33.2 Run B | `max_iterations_exceeded` | 2 | 10 | success prose | completed | `run-record.md` §4–§5 |
| **E33.4** | `max_iterations_exceeded` | 2 | 10 | success prose | **completed** | `run-record.md` |
| `P7-M26-E26.3-PROVE` | `completed` | 0 | 7 | **flat refusal** | **completed** | `P7-M26-E26.3__delivery-notice.md` §DoD |
| `P9-M31-E31.1-PROVE` | `completed` | 0 | 8 | **blocked-permissions prose** | **completed** | `P9-M31-E31.1__demonstration-evidence.md` |
| `P9-M31-E31.2-PROVE` | `max_iterations_exceeded` | 2 | 10 | success prose | completed | `P9-M31-E31.2__run-record.md` (`status: converged`) |

**Two consequences, both material:**

1. **`final_answer` is wrong in both directions.** `P7-M26-E26.3-PROVE` succeeded — the delivery
   notice records *"converged attempt 1/3"*, `bin/ai-project-version` model-produced, proving-vehicle
   test `2 passed` — while its `final_answer` is verbatim **"I'm sorry, but I can't assist with that
   request."** The same notice records that `final_answer` was **"explicitly not consulted."**
   `P9-M31-E31.1-PROVE` likewise converged on attempt 1 of 3 while its prose reports blocked
   permissions. **A mechanism reading `final_answer` as a positive or negative signal fails these.**
2. **The corpus is severely imbalanced: five *completed*, one *did not complete*.** **A mechanism
   that always answers *completed* scores 5/6.** This is a far sharper statement of the limit than
   "two cases do not generalize", and **E39.2 must state it.**

### F9 — Prior art already chose a signal, and it has its own recorded failure mode

Both P7 and P9 determined success from **the QA `validation_command`'s exit code — an out-of-run
verification**, not the agent's own exit code:

- `P7-M26-E26.3__delivery-notice.md`: *"Success determined by the QA `validation_command` exit code +
  transcript, confirmed in the run record; `final_answer` explicitly not consulted."*
- `P9-M31-E31.2__run-record.md`: *"dev's own exit code said 'did not converge,' yet the task was, in
  fact, done correctly, and the QA `validation_command` exit code — the only signal this Epic's
  design treats as authoritative — correctly recognized success regardless."*

**But that signal is itself falsifiable, and this repo records it failing.** The same E31.2 run
record: all three loop attempts returned **QA exit 1 regardless of the dev exit code**, because the
validation command's `$(cat …)` found **no `cat` on `PATH` inside the local-fallback subprocess** —
command substitution returned empty and `test` always failed. Re-running the command manually against
the on-disk file passed. The record's own words: **"Not a real QA failure."**

> **This is `P11-GH-2`'s environment axis, landing on the one signal the prior art trusted.** An
> out-of-run verification is a strong candidate input **and it is not a free oracle** — it inherits
> the environment it runs in. **If the mechanism uses one, its environment must be part of the
> recorded evidence.**
...
- [ ] **`final_answer` is not used as a signal in either direction** (F8) — a run whose
      `final_answer` is a flat refusal may still be *completed*
- [ ] **If an out-of-run verification is used, the environment it ran in is part of the recorded
      evidence** (F9) — the prior art's authoritative signal has a recorded environment-induced
      false failure
...
- **Description:** E39.1's design space after the planning-time measurements. **Three signals are
  ...
  and everything in M40. Proposed-track Structural diagram (AOG §17.3/§17.6), Mermaid, no ComfyUI.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-08-15 | Initial spec. Accepted by the Phase Chat at Stage 2. |
| **1.0.1** | **2026-08-15** | **Added F8 and F9, and withdrew `final_answer` as a tell.** Measured while planning E39.2: the preserved corpus is **six** schema-identical runs, not two, and three of them falsify `final_answer` in both directions — `P7-M26-E26.3-PROVE` returns a flat refusal on a run whose ground truth is *completed*. Added the corpus **imbalance** finding (five *completed* : one *did not complete*; always-*completed* scores 5/6). Added F9: the prior art's authoritative signal is the **out-of-run QA `validation_command` exit code**, which this repo records failing for an environment reason. Two DoD items added. **No change to the mechanism decision, the QA-pass decision, scope, sequencing, or any binding constraint.** Notified to the Phase Chat at handoff. |

**Mid-flight amendment handling (Milestone Starter):** this spec was amended before any
`epic/P11-M39-E39.1` branch existed and before dispatch, so **no running session was reached into.**
