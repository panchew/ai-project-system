...
- **E38.6 — Local/paid controlled comparison.** *(Relocated from M37/E37.1 by HQ Ruling 2026-08-06,
  Decision 3 — the CFO's split-posture intent preserved, not discarded.)* Run one epic's work
  agentic/local and its sibling manual/paid, and compare. **It lands here because here it is native:**
  the adapter surface exists (E38.2), OpenCode is the engine (A1.1), and the work is code-shaped —
  the shape the B3.1 engine comparison measured `qwen3-coder:30b` at its strongest on, where M37's
  dense-prose amendments are its weakest. In M37 the same comparison required scaffolding around an
  engine under retirement assessment, an unsandboxed run across ten governance documents, or M38 work
  pulled forward across a binding order. **Depends on B2.1** (sandbox endpoint forwarding) having
  landed.
...
- **E40.1 — Serialized-lane scheduler.** One reasoning job at any instant; enrollment and concurrency
  kept as separate axes. **Consideration, not scope (added v1.1.2):** a dispatcher that decides *when*
  a run happens is the natural owner of *which git worktree it gets*. `chat-hierarchy.md`'s
  one-worktree-per-concurrent-chat rule has existed since P5-M20-E20.2 and was measured **unobserved**
  on 2026-08-07 — one shared tree, four occurrences in P11 — so it is a practice, not a mechanism.
  Allocating a worktree per dispatched run would be the first thing in this phase to convert an
  interim practice into a mechanism rather than adding another. E40.1 decides whether to take it.
...
| Version | Date | Change |
|---------|------|--------|
| 1.1.2 | 2026-08-07 | **Consideration added to E40.1; no scope change.** Closing the **P4-M19 escalation notice** (2026-06-20, `status: open` for seven weeks across six phase closures) found all four of its asks implemented in **P5-M20** — the git-tracked prerequisite check (`c54efba`), the working-tree isolation convention (`f4c3a30`), and the CFO/Creation scope-direction routing rule — with the third ask moot. A record defect, not an open question. **The live finding is Gap 2's:** that convention exists and is **not observed** — measured 2026-08-07, `git worktree list` returns one shared tree, with four occurrences in P11 including HQ's own. The notice predicted the cost would grow *"as the system moves toward concurrent/agentic execution"*, and `drivr` now exists (E38.1) with M38 Stage B running five epics across two repositories. Recorded against **E40.1** as a consideration — a dispatcher that decides *when* a run happens is the natural owner of *which worktree it gets* — **not placed as scope**; E40.1 decides. It is the first candidate in this phase that would convert an interim practice into a mechanism rather than adding a fifth. Touches §Milestones→M40 (E40.1) only. |
| 1.1.1 | 2026-08-06 | **E37.1 reverts to manual/paid; the local/paid controlled comparison relocates to M38 as E38.6 — HQ Ruling 2026-08-06**, resolving the P11-M37 escalation (Epic → Milestone → Phase → HQ). **E37.1's agentic/local posture was not executable**: measured by HQ, the sandbox cannot reach `localhost:11434` (**HTTP 000**) though it reaches `172.17.0.1:11434` (**HTTP 200**); `run_in_sandbox()` forwards only `AI_PROJECT_ACTIVE_MODEL`; and `check_local_availability` (221) precedes `discover_runner` (252), so dispatch exits **class 5** having done nothing. Documented in P7-M26-E26.3 on **2026-07-12** and worked around per-epic ever since. **HQ added the measurement that decides it: with the endpoint fixed, `local-agent-runner` is STILL ABSENT from the image** — so the endpoint fix alone does not unblock E37.1, and the posture carries a hard dependency on M38's adapter surface. **Route A (unsandboxed) declined** — unsupervised local writes across ten governance documents with G2 existing because the exit code will not reveal failure. **Route B.2 (install the engine into the image) declined** — it invests in an engine under directed retirement assessment (A1.2, E38.4); revisit only if E38.4 retains it and the adapter does not cover sandboxed dispatch. **The endpoint gap is authorized as Bugfix B2.1 (High)** — it edits no governance document and something is demonstrably blocked, so both limbs of the 2026-08-04 B-series refusal are absent. **M37 is not blocked**: E37.2 proceeds, merge order stays E37.1 first. The split-M37 recommendation is confirmed **spent** (it addressed the seven-epic condition the restructure removed) and now attaches to M38. The *"code-shaped epics test the local lane"* line needed no correction here — the v1.1.0 renumber had already moved it to M38 in the starter, which is where it lives; checked rather than assumed. `P11-GH-2` filed (verification performed at the wrong layer; two of its three instances are HQ's). Touches §Milestones→M38 (+E38.6). No milestone ordering or scope boundary otherwise changes. |
