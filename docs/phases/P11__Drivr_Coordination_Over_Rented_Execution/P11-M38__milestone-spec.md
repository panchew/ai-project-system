...
- **G1 — remove derivation steps.** Any input with exactly one non-uniform element gets that element
  **quoted verbatim** rather than described.
  ...
  > **⚠ The count-error tally is reconciled at v1.1.4, and the fix is to stop treating it as a running
  > integer.** Found by the E38.6 Epic Chat; the tally lives in this spec, so the reconciliation is the
  > Phase Chat's.
  >
  > **The collision.** Two *different* defects were each independently numbered **nine**, by two
  > different chats, each correctly adding one to the same recorded base of **eight**, neither aware of
  > the other:
  >
  > | Instance | Defect | Numbered by | Where |
  > |---|---|---|---|
  > | **9a** | `ai-project-yml-spec.md` §4 rule 3 says *"All four required fields"* and lists **five** | Milestone Chat, 2026-08-08 | E38.3 spec, lines 100 / 656 |
  > | **9b** | E38.5 Finding 1's CEILING row overstated by 58,247 bytes (`64,345 − 6,098` — the phase spec counted at full size in a row labelled §P11.3-only) | **Phase Chat**, 2026-08-10 | E38.5 spec line 94, and my own E38.5 review |
  >
  > **Both increments were individually correct and jointly wrong.** This is the
  > *inventory-is-a-floor* failure **occurring inside the counter of inventory failures**, by the exact
  > mechanism the earlier instances established: **a shared number incremented from a stale base.**
  >
  > **Reconciled: at least TEN distinct instances as of 2026-08-10** — the recorded eight, plus 9a and
  > 9b. **The collision itself is an eleventh** if a defect in the record counts, and that question is
  > not worth adjudicating, which is the point.
  >
  > **The remedy is mechanical, not arithmetic.** **Stop citing ordinals.** An instance is identified by
  > **artifact + defect** — *"§4 rule 3's four-versus-five"*, *"E38.5's ceiling row"* — never by its
  > position in a running count, because position shifts when anyone else increments. Any statement of
  > the total is **a floor, carrying its date, its base, and what was counted.** *The record names the
  > disposition; the identifier names the origin* — third application, now to the counter itself.
  >
  > **What this does to G1's argument: it strengthens it.** The tally's own defect was produced by **two
  > paid frontier chats being individually right and jointly wrong** — the failure a verbatim-quoting
  > rule removes, and further evidence that **it was never about model tier.**
...
| Version | Date | Change |
|---------|------|--------|
| 1.1.4 | 2026-08-10 | **The count-error tally is reconciled, and the remedy is to stop treating it as a running integer.** Found by the E38.6 Epic Chat; the tally lives in this spec so the fix is the Phase Chat's. **Two different defects were each independently numbered "nine"**, by two different chats, each correctly adding one to the same recorded base of eight, neither aware of the other: **9a** — `ai-project-yml-spec.md` §4 rule 3 says *"All four required fields"* and lists five (Milestone Chat, 2026-08-08, E38.3 spec lines 100/656); **9b** — E38.5 Finding 1's ceiling row overstated by 58,247 bytes (**Phase Chat**, 2026-08-10, E38.5 spec line 94 and my own review of it). **Both increments were individually correct and jointly wrong** — the *inventory-is-a-floor* failure **occurring inside the counter of inventory failures**, by the mechanism the earlier instances established: a shared number incremented from a stale base. **Reconciled at a floor of TEN as of 2026-08-10**; the collision itself is an eleventh if a defect in the record counts, and that question is deliberately not adjudicated. **The remedy is mechanical: cite instances by artifact + defect, never by ordinal**, since position shifts when anyone else increments; any total is a floor carrying its date, base and scope. *The record names the disposition; the identifier names the origin* — third application, now to the counter. **G1's argument is strengthened, not weakened:** the tally's own defect came from **two paid frontier chats being individually right and jointly wrong**, which is the failure a verbatim-quoting rule removes. Also accepted from E38.6's set: **the escalation instruction** (if E38.2 left no dispatch path, E38.6 escalates rather than scaffolding one — the M37 lesson as an instruction), **B3.1's confound inherited one level up**, and **the GPU contention with E38.5** recorded so the two Stage-B epics sequence rather than collide. **No epic, ordering, gate or scope boundary changes.** |
| 1.1.3 | 2026-08-08 | **Corrections from the E38.3 Epic Chat's Stage-1 set, all five findings verified by the Phase Chat.** **(a) v1.1.1's "enrollment is `init` plus two recorded additions" was an UNDERCOUNT and it was the Phase Chat's.** `bin/ai-project-init` writes `project.name`, `governance.source`, `governance.version`, `submodule_path` — and **§4 rule 3 requires five fields**, so init also omits **`governance.ref`** and **`project.description`**. **Enrollment is `init` plus FOUR additions**: `framework_version`, `models:`, `governance.ref`, `project.description`. **Live consequence: Drivr would be born §4-invalid by the framework's own enrollment tool.** E38.3 found it and **surfaced it rather than absorbing it** — correctly, since it is neither E38.3's to fix unilaterally nor E38.1's fault. **E38.1 must add all four; the Milestone Chat amends E38.1's spec**, which is accepted but not yet executed. **(b) `bin/ai-project-init`'s own validation is a strict subset of §4** — it checks only `project.name`, `governance.source`, `governance.version` (lines 111–114), never `governance.ref` or `project.description`, **so the enrollment tool cannot detect that it produces invalid output.** **(c) Verified: 4 of 12 enrolled configs are §4-invalid on required fields alone** — `ai-project-system-mcp`, `courtis`, `home_finance` (missing `project.description`) and `social-stories-creator` (missing `governance.ref` **and** `project.description`). **(d) §4 rule 3 miscounts its own requirement** — `ai-project-yml-spec.md:495` says *"All **four** required fields"* and then lists **five**. **E38.3 edits that very section and fixes it in passing.** Another instance of the count-omission class, this time inside the normative document. **(e) P10-GH-1 is a class of at least four undefined fields** — `framework_version` (6/12), `created_at` (4/12, **written by init**), `submodule_path` (4/12, **written by init**), `cfo_review_gate` (2/12, prose aside only) — **and §4's unknown-key rules (11, 16, 22) cover only `overrides`, `models` and `visual_artifacts`, so there is no rule at all for unknown keys at the top level or inside `governance:`/`project:`.** **The fold-in stays narrow: `framework_version` only.** Widening to the other three is **not authorized and not the Epic's to take** — it recommends and escalates instead, with the measurement recorded so whoever rules does so in possession of it. The general unknown-key policy **is** the Epic's call. **(f) Finding 5** — `ai-project-system`'s own missing `framework_version` has a strong available reading (the governance source points `source` at its own `./governance`, so `governance.version` *is* its framework version); **the Epic decides and records it**, and the tension is noted honestly rather than smoothed. **No epic, ordering, gate or scope boundary changes**; six epics, split still declined. |
