...
**Sequencing:** after E39.1's QA-pass decision. **May follow or parallel E39.2 — with one hazard now
measured (v1.0.3).**

> **⚠ Artifact-path collision would DESTROY E39.2's evidence base.** Verified in `bin/run-dev-agent`:
> `ARTIFACT_ROOT = .ai-project/artifacts/agentic-runs`, `transcript_path = artifact_dir /
> "transcript.json"` (line 246) and `run-metadata.json` (line 284). **A QA run dispatched for the same
> `epic_id` writes both files at the same paths and overwrites the dev run's preserved transcript and
> metadata** — the exact corpus E39.2 validates against, including its four held-out cases.
>
> **E39.3's criterion 4 already requires distinct artifact paths that overwrite nothing, so the epic
> handles it.** Recorded here because **the risk is cross-epic**: run in parallel with E39.2 and
> without that requirement satisfied first, E39.3 could silently delete the evidence E39.2 depends on.
> **Distinct paths must be in place before any QA run executes.** The preserved archive uses suffixed
> names (`transcript-A-qwen2.5-coder-14b.json`) while the code writes plain `transcript.json` — so the
> archive was hand-curated, which makes the collision **more** likely to recur, not less.
...
| Version | Date | Change |
|---------|------|--------|
| 1.0.3 | 2026-08-16 | **A cross-epic destructive hazard recorded from E39.3's set, verified independently.** `bin/run-dev-agent` writes `transcript.json` (line 246) and `run-metadata.json` (line 284) under `.ai-project/artifacts/agentic-runs/<epic_id>/`. **A QA run dispatched for the same `epic_id` overwrites both** — destroying the dev run's preserved transcript and metadata, which is **exactly the corpus E39.2 validates against, including its four held-out cases.** E39.3's criterion 4 already requires distinct artifact paths that overwrite nothing, so the epic handles it; recorded at milestone level because **the risk is cross-epic** and this spec permits E39.3 to run parallel to E39.2. **Distinct paths must be in place before any QA run executes.** Note the preserved archive uses suffixed filenames while the code writes plain ones — the archive was hand-curated, which makes recurrence **more** likely. Also accepted from E39.3's set: **the five-criterion bar defining a genuine `epic_qa` run** (role assesses not produces; tools cannot mutate the tree; routing provably from `models.epic_qa`; capture committed at distinct paths; verdict advisory only, binding to E39.1's non-authority decision) — which milestone constraint 4 required in effect but never defined, **so both outcomes are now precise**; and the **bounded attempt with a hard gate** — reuse `run-dev-agent`'s machinery, change only task text, tool set and artifact paths, touch Drivr not at all, and **if the bar cannot be met within scope, stop and report G11 open with the specific reason rather than widening scope to force a close.** **No epic, ordering, constraint or scope boundary changes.** |
