...
> **⚠ The two governing documents render these differently, and both claim to be verbatim.** The Epic
> Spec's §Scope-1 table reads as above. The **Epic Execution Chat Starter's** inline parenthetical
> reads *"though it can be brought back"* and omits *"Enrolled in the registry."* from Active. **The
> starter outranks the spec** in the governance hierarchy. The registry reproduces the **spec's table
> form**, on the reading that the starter's parenthetical is a compression of it, and **records the
> discrepancy in the file itself** rather than picking one silently. Reported — it needs a correction
> to one document, not a ruling.
...
> invalid.** The tool validates what it is pointed at; separating projects from worktrees is the
...
| 14 | **The two governing documents render the CFO's state definitions differently**, and both claim verbatim. | **Left.** Reported; needs a correction to one document, not a ruling. |
...
**Baseline reconciliation: 393 (inherited) + 79 (validator tests) + 16 (registry tests) = 488.**
...
configuration is involved** (AOG §16.3/§16.8).
...
graph TB
    ...
    subgraph BUILT["BUILT in this repo"]
        ...
        T["tests — 79 validator + 16 registry"]
    ...
    subgraph OUT["ESCALATED — not acted on"]
        ...
        E4["4 · state definitions rendered<br/>two ways in two documents"]
    end
...
| Three states match the CFO's definitions verbatim | ✅ §3 (+ discrepancy reported) |
...
| Validator carries its own tests, running in this repo's suite | ✅ 79 + 16 |
...
| Suite green, exact command stated, `P10-GH-10` recorded with both results | ✅ §12 — **488 passed** |
