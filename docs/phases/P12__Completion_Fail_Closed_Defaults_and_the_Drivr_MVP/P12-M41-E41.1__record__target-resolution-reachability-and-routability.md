...
### 9.5 What is still NOT resolved

**Which route each row should use is a CFO decision, not a measurement**, and this epic does not make
it:

- **`phase`:** `openai/gpt-5.6-sol` (self-reports the product name) vs `openai/gpt-5.6-sol-fast` vs
  `opencode/gpt-5.6-sol`. **Recommended: `openai/gpt-5.6-sol`**, because it is the one that states
  the ruled product name itself.
- **`milestone`:** `opencode/deepseek-v4-flash` vs `opencode-go/deepseek-v4-flash`. Both resolve;
  **only the first was made to answer.**
- **The `.ai-project.yml` strings would be `remote:gpt-5.6-sol` and `remote:deepseek-v4-flash`**,
  each matching its self-report's bare id under the same locality-prefix strip the check already
  applies to `remote:claude-opus-5`. **Writing them is E41.5's, behind its two gates — not this
  epic's.**
- **The snap-path dependency is real and unrecorded anywhere.** Any future session driving OpenCode
  from inside VS Code will see **zero credentials** unless `XDG_DATA_HOME` is corrected. That is a
  live trap for M44 and for Drivr, and it is escalated.
