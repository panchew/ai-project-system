...
> (PR #221; **not yet on `master` and therefore not yet on this branch** — see Notes)
>
...
- **On the R6 ruling's own citation, which does not yet resolve on this branch.** PR #221 is open and
  awaiting the CFO's §11.6.1 diff review, so the ruling file is **not on `master` and not on
  `milestone/M41`.** The M41 spec v1.2.0 cites it by filename regardless. **This is the same
  dangling-citation shape recorded in the milestone spec's Notes** — and this time the Phase Chat
  **said so in advance** rather than leaving it to be discovered. **Confirm the file exists on your
  branch before following the citation** (`git cat-file -e <rev>:<path>`); if it does not, the
  ruling's terms as quoted in the box above are the operative statement, and the branch will be
  re-synced once #221 merges.
