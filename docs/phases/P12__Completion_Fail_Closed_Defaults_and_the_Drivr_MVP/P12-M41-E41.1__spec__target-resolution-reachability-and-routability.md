...
### R6 — **RULED AND UPHELD, 2026-08-20.** F6's shape is not unique to `epic_manual`

> **RESOLVED — HQ Ruling, 2026-08-20**
> `.ai-project/artifacts/rulings/2026-08-20__ai-project-system-hq__ruling__r6-manual-verification-surface-rule.md`
> (PR #221, **merged**; `master` at `f31ec78`. **The file resolves on `milestone/M41`** — verified
> with `git cat-file -e`, 2026-08-20. See Notes for how it got there.)
>
> **R6 was upheld as a correction to HQ's own F6 ruling.** F6 identified the right mechanism and
> scoped it to one row — and fixed the least severe of three by blast radius: `epic_manual` reaches
> manual Epic chats, `milestone` reaches **every Milestone Chat**, `phase` reaches **the Phase Chat
> itself.**
>
> **The rule, which now governs this epic's D2 Part B:**
>
...
> > `epic_dev`/`epic_qa`** — the dispatch lanes are out of scope and land on E41.3's evidence alone.
>
> **What this does to this epic: D2 Part B stops being informational and becomes load-bearing.**
> Under M41 spec v1.2.0, E41.5 lands **at most one** verification target — `creation`, **and only if
> E41.1 confirms both halves.** `phase`, `milestone` and `epic_manual` become **one carry-forward
> under one trigger**, CFO-owned, no expiry, with F6's subsumed rather than superseded.
>
> **The rows are not re-decided.** Only the landing is gated — **on a fact about surfaces, not a
> judgment about models.** And **every measurement obligation is untouched**: a row that will not
> land is still measured.

**The original finding, as filed, is preserved below.**
...
- [ ] **D2** — The reachability record: Part A (target answers) and **Part B — BINDING** (surface
      runs it / emits a self-report at all / that self-report is readable and matches — **three
      columns**, R6a), separately recorded, dated, with the check layer stated. **Part B decides
      whether `creation` lands in E41.5; it is an acceptance criterion, not a finding**
...
- [ ] **BINDING (HQ Ruling on R6, 2026-08-20 — promoted from a finding to a criterion): for every
      moving manual verification target, BOTH halves of the manual-surface rule are confirmed or
      explicitly recorded as not confirmed.** Endpoint reachability alone **does not satisfy this
      criterion.** An epic that measures Part A and leaves Part B unanswered **has not delivered**,
      because E41.5's deliverable 1 is decided by this result: `creation` lands only if both halves
      are confirmed here
...
**Blockers**
- None known. **R6 is RULED** — upheld 2026-08-20 as one manual-surface rule — and is **not** a
  blocker on this Epic's own work. **It is the reason D2 Part B is BINDING**: this Epic measures the
  condition, and **E41.5 lands `creation` only if both halves are confirmed here.**
...
- **Description:** E41.1's five deliverables and the three planning-time findings that reshaped them.
  ...
  *a chat can be opened on it and self-report an ID the check can read* — because the second is what
  ...
  question this epic answers by running it. Proposed-track Structural diagram (AOG §17.3/§17.6),
  Mermaid, no ComfyUI.
...
- **The undefined fourth state is M44's repair, not this epic's.** HQ ruled it (Decision 3):
  **refuse by default; proceed only on an explicit RECORDED declaration in the session's first
  substantive response; silence never available** — and placed the repair in **M44**, framed for a
  corpus where **Claude Code is one surface among several**, because three of five verification
  targets ultimately move off it (R6b). **This epic does not implement that rule and does not amend
  `chat-hierarchy.md`.** It records the observation the repair will need: **per target, does the
  surface emit a self-report at all**, separately from whether it matches. **The second observation
  is useless without the first.**

- **On the R6 ruling's own citation — DISCHARGED, and the account of how matters more than the
  fact.** For part of 2026-08-20 this spec, the milestone spec and the Starter cited
  `…__ruling__r6-manual-verification-surface-rule.md` **by filename against a file absent from the
  branch** — four dangling citations, because #221 had not merged. **It merged** (`master` at
  `f31ec78`), the Phase Chat synced `master` → `phase/P12` → the three milestone branches, and the
  file now **resolves on `milestone/M41`** — verified here with `git cat-file -e`, not taken on
  report.

  **How it was caught is the part worth keeping: it was not.** Nothing in this project detected the
  staleness. HQ told the Phase Chat, and the repair was manual. **Downward amendment is mechanised;
  upward branch staleness is unowned**, and the only routine that would currently have caught it is
  a per-session `git rev-list --count origin/milestone/M41..origin/master`. **Run it before you
  start and before you deliver.**

  **And confirm any cited artifact exists on your branch before following it** — `git cat-file -e
  <rev>:<path>`. **A dead citation is not a typo; it is the defect**, and this milestone produced
  three of them from three different causes.

- **On R6, and why it was filed rather than absorbed.** The M41 spec's F6 was escalated by the Phase
...
| Version | Date | Change |
|---|---|---|
| 1.0.4 | 2026-08-20 | **Corrects one stale line that survived v1.0.3's own correction**: Dependencies → Blockers still described R6 as *"a finding under escalation"*. **R6 was upheld and ruled on 2026-08-20**, and the line now says so plus the live consequence — R6 is **why D2 Part B is binding**, since E41.5 lands `creation` only if this Epic confirms both halves. **Derived-claim rot, and the instructive part is where it survived:** v1.0.3 corrected the R6 *citation* claim in the Notes and did not sweep the other line resting on the same premise. **A correction that fixes the instance in front of it, rather than the premise's dependents, leaves rot behind** — which is the same shape as F3's file list surviving two amendments. No scope, deliverable or criterion change. |
| 1.0.3 | 2026-08-20 | **Corrects a claim in this spec that its own premise falsified — the rot class this milestone has been naming, applied to itself.** v1.0.2 recorded that the R6 ruling's citation *"does not yet resolve on this branch"* because **#221 was unmerged. #221 merged** (`master` `f31ec78`), the Phase Chat synced `master` → `phase/P12` → `milestone/M41`, and the file now resolves — **verified here with `git cat-file -e`, not taken on report.** The ruling box and the Note are corrected rather than left standing beside a changed fact. **The Note keeps the account rather than only the outcome:** nothing detected the staleness, HQ told the Phase Chat, the repair was manual — **downward amendment is mechanised, upward staleness is unowned**, and the per-session drift count is currently the only routine that would have caught it. **No deliverable, target, criterion or line-up value changed.** |
| 1.0.2 | 2026-08-20 | **Amended for the HQ Ruling on R6** (`…__ruling__r6-manual-verification-surface-rule.md`, PR #221), which **UPHELD R6 as a correction to HQ's own F6 ruling** and replaced per-row adjudication with **one manual-surface rule**: no manual verification target lands a new value until a surface is confirmed to **run that model AND emit a self-report the E31.3 check can read**, scope the five verification targets and **explicitly not `epic_dev`/`epic_qa`**. **Consequence for this epic, and it is a promotion in force rather than a change in work: D2 Part B stops being informational and becomes BINDING** — a new acceptance criterion requires both halves confirmed, or explicitly recorded as not confirmed, for every moving verification target, because **E41.5 now lands at most one of them (`creation`) and only if E41.1 confirms both halves.** Records that the **undefined fourth state is M44's repair, not this epic's** (HQ Decision 3: refuse by default, proceed only on an explicit recorded declaration, silence never available), and that the ruling's own citation **does not yet resolve on this branch** because #221 has not merged. **Amended in response to the Phase Chat's in-session `P11-GH-1` step-2 notification of milestone spec v1.2.0 (`fc65019`); the named sections were re-read at v1.2.0 before this amendment. No deliverable added or removed; no target, namespace or line-up value changed.** |
