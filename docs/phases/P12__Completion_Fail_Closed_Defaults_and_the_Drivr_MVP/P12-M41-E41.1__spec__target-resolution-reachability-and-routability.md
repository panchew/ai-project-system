...
- whether that surface emits a **self-report the P9-M31-E31.3 check can read**;
- and, where either half is unknown, **say unknown.** An unknown recorded as unknown is this epic
  delivering; an unknown rendered as a checkmark is the failure this milestone exists to prevent.
...
- [ ] **D2** — The reachability record: Part A (target answers) and Part B (surface exists and
      self-reports), separately recorded, dated, with the check layer stated
...
- [ ] Reachability Part B is recorded for every manual verification target, with *unknown* recorded
      as unknown
...
- [ ] **Every target has a dated reachability result with its check layer stated** — and, per R6,
      the *surface* question is answered separately from the *endpoint* question for every manual
      verification target
