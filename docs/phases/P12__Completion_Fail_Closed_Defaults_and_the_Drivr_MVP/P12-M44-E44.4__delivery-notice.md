...
merge_details:
  pr_number: <filled after PR creation>
...
> **`pr_number`/`pr_url` are filled once the PR exists.** The remaining `merge_details`
> fields — `merge_commit` / `merge_timestamp` / `merge_strategy` — describe a merge that
> has not happened and are **unfillable at delivery time** (`P10-GH-4`, restated, not
...
citation-vs-example distinguished: PSG references, document-internal `§N` item numbers,
and every fenced quoted example are untouched — the AOG's fenced example blocks are
**byte-identical** to the branch point (shown, not asserted). **D4** — version
...
| **Refs** | Branch point `97afdd6` (merge-base with `milestone/M44` verified identical). Suite at the branch point and on this branch: **741 passed / 0 failed** at the branch point; **752 passed / 0 failed** with E44.4's D5 check (`PYTHONPATH=. pytest -q`, bare `pytest` fails collection). **+11 tests, no skip introduced.** The Starter's Suite Baseline (740) is stale — re-measured on `milestone/M44` at `97afdd6`: 741 (consistent with E44.3's notice). |
...
**The rule deciding citation vs example (stated):** a `§N` token is an AOG citation only
when it belongs to an `AOG` citation run (load lists `AOG preamble+…`, `AOG §N`), is an
AOG-only token (`§16.x`), or is a continuation of an `AOG preamble` list; a token inside
a real fence, a PSG reference, or a bare `§N` used as a document-internal item number
(e.g. a Delivery Notice's own `✅ §N` table) is **not** a cross-reference and was not
rewritten. The AOG's own internal cross-references (e.g. `§12 "Acceptance Outcomes"` →
`§14`) were updated in the same pass. **No fenced example rewritten — shown:** the AOG's
eight fenced blocks are byte-identical to `97afdd6` (`fenced_blocks(orig) ==
fenced_blocks(current)`), and the `## ` example headings inside them are unchanged.
...
- [x] Suite green — **752 passed / 0 failed**, +11 tests, no skip introduced, invocation
      `PYTHONPATH=. pytest -q` and ref `97afdd6` stated
...
flowchart LR
  ...
  SWEEP["D3 — CROSS-REFERENCE SWEEP, corpus-wide (9b0f0fb)<br/>261 files · governance/ docs/ rulings/<br/>citation vs example: a §N in an AOG run, or §16.x, is a<br/>citation; a §N in a fence / a PSG ref / an internal item<br/>number is NOT and was NOT rewritten<br/>AOG's fenced blocks BYTE-IDENTICAL to 97afdd6"]
  ...
  CHECK -. "suite: 752 passed / 0 failed (baseline re-measured 741)" .-> OUT
