...
merge_details:
  pr_number: 272
  pr_url: https://github.com/panchew/ai-project-system/pull/272
...
`models.epic_manual` is **`remote:deepseek-v4-flash`**. **These are not the same model.** Per the
E41.1 record §9.5, `deepseek-v4-flash-vision-exp` is explicitly listed among the models that are
*"different models rather than different routes to the same one."* The configured value
(`deepseek-v4-flash`) and the harness-reported value (`deepseek-v4-flash-vision-exp`) disagree.
Under `advisory`, I state the mismatch plainly and proceed. The work delivered here does not depend
on which of the two models is running — the gate's logic, its tests, and the Drivr suite results
are model-agnostic (they are measured on this host, and the model is recorded as context, never
scored).
