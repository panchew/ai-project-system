...
> **⚠ Defect #1 overlaps E44.4's D5.** E44.4 builds *a check that fails on a duplicate section number
> or title, fence-aware* — that **is** defect #1's heading-uniqueness/monotonicity check. E44.6 must not
> build it twice: the epic coordinates, either citing E44.4's check as defect #1's coverage, or building
> the complement (monotonicity) if E44.4's D5 stops at duplicates. The monotonicity half (a section
> numbered `16` landing before `15`) is the part E44.4's *"duplicate number or title"* wording does not
> name — treat that as either in E44.4's D5 or E44.6's D1, but **once**.
...
- **Description:** E44.6 promotes findings into the tier that owns them. SN-30 Rec 1 builds a mechanical
  ...
  path — no ordinal, the gap left open and unscoped. Proposed-track Structural diagram (AOG §16.3/§16.6),
  Mermaid, no ComfyUI.
...
- **Defect #1 is E44.4's own subject, which is why the two epics must not ship two checks.** The AOG
  duplicate-`§13`/`§14` heading defect is exactly what E44.4 repairs and guards with its D5. The monotonicity
  half (`16` landing before `15`) is the one piece E44.4's *"duplicate number or title"* wording does not
  name — the epic resolves that boundary once, in one epic, not in both.
