...
final_artifacts:
  ...
  - name: The go-to-blocker affordance (D3) — resolves through the registry; refuses to guess on vacant/undetermined
    path: drivr/controls/model.py, drivr/controls/__init__.py (Drivr repo)
    type: code
  - name: The three present controls' guards — 23 tests, each falsified in both directions
...
| **Repo for `git log`** | Drivr HEAD at delivery: **`43276da`** (`test(E46.4): a go-to-blocker with an unparseable subject refuses (fail-closed), not guesses`), on `epic/P12-M46-E46.4`, on top of `98ff15b` (`feat(E46.4): approval by signed one-time link, escalation one level, go-to-blocker`), on top of `99392ed` (`main`). |
| **Suite (Drivr, operative)** | `python3 -m pytest -q` from the Drivr root. **Baseline at the branch point: 527 passed / 0 failed** (G2 re-measure at `99392ed`, post-E46.1+E46.2+E46.3; the planning baseline was 471 at `4872107`, but Drivr moved). **After: 550 passed / 0 failed** (527 + 23 new in `tests/test_controls.py`). **No skips introduced.** The `ai-project-system` suite does not cover Drivr and is not a measurement of anything this epic produced. |

**P11-GH-2 note — a deviation, stated (and on the record via `P12__carry-forward-note__drivr-no-remote-recoverability.md`).** This Drivr has **no configured remote** (`git remote -v` is empty), per the carry-forward note (M45, Phase-owned open question). The Drivr commit (`43276da` on `epic/P12-M46-E46.4`) exists **locally only** — no push could be made or push-verified, and **no GitHub PR for the Drivr change can be opened**. The committed content is the source of truth; the parent's merge of `epic/P12-M46-E46.4` → `main` is what completes BC9, and I say so rather than pretending a push happened. This matches the E46.1/E46.2/E46.3 deliveries' handling.
...
`..._does_not_hardcode_an_address`, `..._states_the_terminus_at_hq`,
`..._on_an_unparseable_subject`, and the shape test
...
- [x] The controls and their tests are on **Drivr `main`** — **not yet met by me, and cannot be met by me**: §11.6 makes the parent (the M46 Milestone Chat) the party that merges `epic/P12-M46-E46.4` → `main`. The code is committed on the epic branch (`43276da`) and ready for that merge. Stated as an open item, not marked done on the branch (the M43/M45 defect was exactly *finishing on the epic branch*).
- [x] Drivr suite green at **550** with the invocation (`python3 -m pytest -q`) and the ref (branch point `99392ed`; delivery `43276da`) stated — baseline 527 at the branch point (G2), 471 at `4872107`
...
After each revert, `tests/test_controls.py` returned **23 passed**, and the full Drivr suite returned
**550 passed / 0 failed**.
...
(`43276da` on `epic/P12-M46-E46.4`) exists locally but **was not pushed or push-verified**, and no
