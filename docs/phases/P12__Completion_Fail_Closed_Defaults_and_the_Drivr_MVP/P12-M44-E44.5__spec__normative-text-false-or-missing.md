...
|---|---|
| Constraints, both lines (*"do not have live access"*, *"no automatic synchronization… possible"*) | **Struck.** Replaced by a **dated** factual statement of current capability, plus the rule that a Constraints section carrying a technical claim is **re-checked whenever the document is versioned**. |
...
| *"Manual Enforcement … not automation"* (and the overlapping Constraints line *"All governance enforcement is by reference and manual review"*) | **Struck; replaced** by the narrower true statement: *automated checks do not confer acceptance* — a passing check is evidence; a governed parent or human still accepts. |
...
- **The three-Constraints-vs-two overlap is the one place the ruling does not spell out the targets, and
  the epic must.** Decision 7 quotes **two** false Constraints, but the document's Constraints section
  holds **three** lines — the third, *"All governance enforcement is by reference and manual review,"*
  is the "Manual Enforcement… not automation" theme in constraint form, and travels with the struck
  "Manual Enforcement" statement, not with the *"does not propagate"* survivor. Mapping the ruling to the
  actual lines is exercise, not re-decision; a mis-mapping that leaves the third line standing would
  restore the falsity the amendment removes.
