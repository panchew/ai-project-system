...
> **This epic stays superseded.** Its purpose shifts from *deciding* a landing to *characterizing*
> one that happened. **Whether it is dropped or retained is still the Phase Chat's.**


> # ⛔ THIS EPIC HAS NO SUBJECT MATTER — CFO RULING, 2026-08-23
>
> **All three Epic keys are set to `local:qwen3-coder:30b`.** `epic_dev` and `epic_qa` **already
> hold that value**, so **neither row moves.**
>
> **There is no candidate to measure and nothing to compare.** This epic existed to choose between
> `qwen3-coder:30b`, `qwen3.6:27b` and `qwen3.8:27b` for the two lanes. **The CFO has chosen, and he
> chose the incumbent.**
>
> **His stated priority:** *"being able to execute levels with their chosen model"* outranks deciding
> whether `qwen3.8:27b` should replace the lanes. **That is a line-up decision and it is his.**
>
> **What this does NOT do — the findings survive their epic:**
> - **The ruled bimodal bar stands.** *Non-reproducible → `UNDETERMINED`; raise N; if unstable, hold.*
>   It cost nothing to fix in advance and it is on the record for the next comparison.
> - **The lane-vs-model ruling stands.** Off-lane evidence cannot move a lane row.
> - **The parse defect is unchanged and still unowned** — `local_agent_runner/tool_calls.py:171`, a
>   third repository. **It no longer gates M41**, because no lane row is moving. **It is
>   infrastructure, which is what it always was.**
> - **E41.2's instrument survives entirely** and is the durable output of this line of work.
>
> **✔ DISPOSITION ANSWERED — M41 spec v1.5.0, `a075114`, 2026-08-23. RETAINED, NOT DROPPED.**
> Verified on the branch: the frontmatter reads `- E41.3  # SUPERSEDED 2026-08-23 by CFO ruling —
> retained, not dropped`, and the Epic Detail carries the stop banner and the four survivors.
> **Dropping it would erase a decision and orphan four findings citing an epic the milestone would
> then say never existed** — the same reasoning as row P4's closure, recorded **beside** the row
> rather than by moving it. **This spec stays superseded and no work starts against it.**
...
### ✔ (a) RULED BY THE CFO, 2026-08-23 — a baseline that is not reproducible is not a baseline

**Escalated by E41.2, carried by this milestone, routed by the P12 Phase Chat as Binding Constraint
6's territory, and ruled by the CFO** under his stated criterion: *the option that makes the system
more reliable, stable and performant in the long run.*

> **THE RULED BAR FOR A BIMODAL BASELINE**
>
> **Non-reproducible → `UNDETERMINED`.** Bimodality is **a first-class result, not something to
> aggregate around.**
>
> 1. **Raise N until the incumbent is stable.**
> 2. **If it does not stabilise, record it as unstable and HOLD the row.** That is a delivered
>    result, not a failure to reach one.
> 3. **Once stable, apply PAIRWISE NO-WORSE** — the candidate's **worst ≥ the incumbent's worst**
>    *and* the candidate's **best ≥ the incumbent's best.** It cannot be gamed by a lucky run.
>
> **Rejected, and recorded so the choice is auditable:** **worst-of** (incumbent `0/20` — almost any
> candidate clears; rows move on the incumbent's worst moment) and **best-of** (incumbent `9/20` —
> compares a candidate's typical against the incumbent's best). **Both were eliminated by the
> criterion before the choice among the survivors was made.**

**This is the ratified repair's SIXTH substrate in this phase** — after S3's unloadable replay case,
R6a's undefined fourth state, the tie-break's empty, `undetermined` as its own board state, and
U8's zero-credentials case. **When the evidence cannot distinguish, say so; do not pick.**

#### What it costs this epic, stated plainly

**The bar is now committed and the data does not exist yet — which is the correct order** and the
reason the CFO ruled in the abstract rather than after re-measurement.

**But raising N on the CURRENT dispatch path would spend runs twice.** Under the lane ruling above,
nothing measured off-lane can move a lane row, so **a stable baseline established on the host is a
stable baseline that still cannot decide anything.**

> **So the stable baseline this bar requires should be established through the REPAIRED LANE after
> M42 closes — not by raising N now.**

> ### ⚠ SUPERSEDED — v2.1.1, 2026-08-23. This chat's inference, and it was wrong
>
> **v2.1.0 said here:** *"the bimodality may not survive that… M42 repairs the lane… the `FAIL 0/20`
> mode may simply not recur."* **That has no support in M42's scope, and the P12 Phase Chat caught
> it.**
>
> **Re-measured here against `origin/milestone/M42`'s spec, literal AND emphasis-stripped, because a
> literal zero is not trustworthy in this corpus:**
>
> | Term | literal | emphasis-stripped |
> |---|---:|---:|
> | `parser` / `parse` / `function=` / `tool-call` | **0** | **0** |
>
> **M42's four defects are:** isolation fails open, unscoped staging, the merge ladder, the
> initializer. **None is tool-call parsing.** `bin/run-dev-agent` appears three times and is a repair
> target in none — twice as the source of the `EXIT_LOCAL_UNAVAILABLE = 5` convention, once inside
> the blast-radius enumeration.
>
> **What survives:** **M42 does change the lane** — E42.1 makes sandbox-absence abort — **so (b)'s
> re-measurement requirement stands unaffected**, and so does *raising N now would spend the runs
> twice.* **Neither depended on the parse changing.**
>
> **What is corrected: EXPECT THE BIMODALITY TO PERSIST.** If `<function=…>` handling produces the
> `FAIL 0/20` mode and nothing repairs it, post-M42 re-measurement shows the same two modes.
>
> **The ruled bar already says what happens then, which is why this is a warning and not a problem:**
> *raise N; if it does not stabilise, record it as unstable and **HOLD the row**.* **Holding is the
> EXPECTED outcome here, not a discovery** — and it is a delivered result.
>
> **How this chat got it wrong, recorded because the shape is reusable.** *"M42 repairs the lane"* is
> **true**, and *"M42 repairs the parse"* does not follow — **a premise true at the wrong
> granularity.** `P11-GH-2`'s shape applied to a **scope** rather than an environment: a claim about
> M42's contents, asserted without reading M42. **And it was written as consolation** — the sentence
> explaining why the ruled bar might cost nothing. **The sentence that softens a constraint is the
> one least likely to be checked.**

~~**And the bimodality may not survive that.** `DEV RUN 2`'s failure came from **the parser** taking
`<function=…>` as prose — a lane property, and M42 repairs the lane. If the repaired path parses that
syntax, the `FAIL 0/20` mode may simply not recur and the baseline may be stable on its own.~~
**The ruled bar was fixed before anyone could see which way it went, and that part stands.**

> **⚠ Larger than this epic, flagged upward by the Phase Chat and not placed here.** If the parse
> defect is what makes the incumbent bimodal, **and nothing owns fixing it**, then `epic_dev` and
> `epic_qa` may be **permanently unqualifiable** — not because a candidate is bad, but because **the
> lane cannot produce a reproducible baseline for anything to be compared against.** **A defect
> determining whether a ruled line-up row can ever land, currently in nobody's scope.** With HQ.
> **⚠ UPDATED v2.1.2 — the parse HAS now been located, and where it lives is the point.**
> HQ located it; **this chat verified both halves independently rather than taking the report:**
>
> | | |
> |---|---|
> | `bin/run-dev-agent` | **a shim.** 293 lines, **one** parse-related line; its own docstring: *"the governance adapter between the P3 orchestrator and `local-agent-runner`… the CONTRACT §7 shim"* |
> | The parse itself | **`local_agent_runner/tool_calls.py:171` — `def parse_tool_call(...)`**, in `local-agent-runner` at HEAD `4ec1e8f` |
>
> **It is in a THIRD REPOSITORY** — not `ai-project-system`, not `drivr`. **That is why nobody owns
> it:** M42 cannot reach it, M41 cannot reach it, and this project's governance does not extend to
> it. **The defect that may make both lane rows permanently unqualifiable is outside the scope of
> every milestone that depends on it.**
>
> **Boundary, stated because a single pattern is not evidence in this corpus:** this chat located the
> function by name across five patterns after one naive pattern returned nothing. **It has not read
> the function or confirmed that this specific code path produced `DEV RUN 2`'s `<function=…>`
> failure.** **What is established: the parse is there, `run-dev-agent` is a shim, and M42's scope
> does not include either.**

### Both blockers, and what each now needs

| Blocker | Status | What it needs |
|---|---|---|
| **(a) the bimodal bar** | ✔ **RULED** 2026-08-23 | nothing further — apply it |
| **(b) lane vs model** | **standing** | **M42 closed, then re-measurement through the repaired lane** |

---

### Superseded: this epic was additionally blocked on (a), which was the CFO's
...
| Version | Date | Change |
|---|---|---|
| 3.0.1 | 2026-08-27 | **Disposition answered and recorded rather than left open.** The Phase Chat retained E41.3 rather than dropping it — M41 spec **v1.5.0, `a075114`**, verified on the branch. **Dropping it would erase a decision and orphan four findings citing an epic the milestone would then say never existed**, which is row P4's precedent: record the closure **beside** the thing rather than by moving it. This spec's *"disposition is the Phase Chat's"* line was a live dependent of a premise that had already moved. |
| 3.0.0 | 2026-08-23 | **SUPERSEDED — this epic has no subject matter.** The CFO ruled all three Epic keys to `local:qwen3-coder:30b`; **`epic_dev` and `epic_qa` already hold it, so neither row moves and there is no candidate to compare.** His stated priority: *being able to execute levels with their chosen model* outranks deciding whether `qwen3.8:27b` replaces the lanes. **The findings survive the epic:** the ruled bimodal bar, the lane-vs-model ruling, the still-unowned parse defect at `local_agent_runner/tool_calls.py:171`, and E41.2's instrument. **The parse no longer gates M41** — no lane row is moving — and is infrastructure, as it always was. **Formal disposition (drop vs retain-as-closed) is the Phase Chat's; this spec is marked superseded and no work should start against it.** |
| 2.1.2 | 2026-08-23 | **The parse has been LOCATED, so v2.1.1's *"not established"* boundary is itself corrected before it could go stale.** HQ located it; **this chat verified both halves independently**: `bin/run-dev-agent` is **a shim** — 293 lines, one parse-related line, docstring *"the governance adapter between the P3 orchestrator and `local-agent-runner`… the CONTRACT §7 shim"* — and the parse is **`local_agent_runner/tool_calls.py:171`, `def parse_tool_call(...)`**, in `local-agent-runner` at HEAD `4ec1e8f`. **It is in a THIRD REPOSITORY, and that is the point: M42 cannot reach it, M41 cannot reach it, and this project's governance does not extend to it — which is WHY nobody owns the defect that may make both lane rows permanently unqualifiable.** **Boundary retained honestly:** located by name across five patterns **after one naive pattern returned nothing**; the function has **not** been read and this path has **not** been confirmed as the one that produced `DEV RUN 2`'s failure. **Nothing else changes** — the ruled bar, (b), and *expect the bimodality to persist* all stand. **Also records the null result of #233 (`master` `ba595e4`, phase spec v1.2.0), verified at this layer: ZERO of this milestone's epic artifacts changed.** |
| 2.1.1 | 2026-08-23 | **Corrects this chat's own inference in v2.1.0, caught by the P12 Phase Chat.** v2.1.0 said the bimodality *may not survive M42* because *M42 repairs the lane*. **Re-measured here against `origin/milestone/M42`'s spec, literal and emphasis-stripped: `parser`, `parse`, `function=` and `tool-call` all score ZERO both ways.** M42's four defects are isolation-fails-open, unscoped staging, the merge ladder and the initializer; **`bin/run-dev-agent` appears three times and is a repair target in none.** **`M42 repairs the lane` is true; `M42 repairs the parse` does not follow — a premise true at the wrong granularity**, which is `P11-GH-2`'s shape applied to a **scope** rather than an environment. **Corrected: EXPECT THE BIMODALITY TO PERSIST**, and the ruled bar already answers that case — *hold the row* — so **holding is now the EXPECTED outcome rather than a discovery.** **Unaffected:** (b)'s re-measurement requirement, and *raising N now spends the runs twice* — neither depended on the parse changing. **Recorded about the failure itself:** the clause was written as **consolation**, explaining why the ruled bar might cost nothing, and **the sentence that softens a constraint is the one least likely to be checked.** Also carries the Phase Chat's upward flag with its boundary: **if nothing owns the parse defect, both lane rows may be permanently unqualifiable** — with HQ, and **where the parse happens is NOT established.** Original text struck through, not removed. |
| 2.1.0 | 2026-08-23 | **(a) RULED BY THE CFO — a baseline that is not reproducible is not a baseline.** Under his stated criterion (*reliable, stable, performant in the long run*): **non-reproducible → `UNDETERMINED`**; **raise N until the incumbent is stable**; **if it does not stabilise, record it as unstable and HOLD the row** — a delivered result, not a failure to reach one; **once stable, apply PAIRWISE NO-WORSE** (candidate's worst ≥ incumbent's worst **and** best ≥ best), which cannot be gamed by a lucky run. **Worst-of and best-of were eliminated by the criterion before the choice among survivors**, and are recorded as rejected so the decision is auditable. **The ratified repair's sixth substrate in this phase** — when the evidence cannot distinguish, say so rather than pick. **The bar was committed BEFORE the data exists**, which is the correct order and the reason it was ruled in the abstract rather than after re-measurement. **Practical consequence recorded: the stable baseline this bar requires should be established through the REPAIRED LANE after M42, not by raising N now** — under the lane ruling nothing measured off-lane can move a lane row, so raising N on the host would spend the runs twice. **And the bimodality may not survive M42 at all:** `DEV RUN 2`'s failure came from the **parser**, a lane property that M42 repairs. **Minor bump: the bar is supplied, no deliverable added or removed.** |
