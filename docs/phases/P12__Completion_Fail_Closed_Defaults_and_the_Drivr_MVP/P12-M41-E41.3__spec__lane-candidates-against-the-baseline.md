...
| files changed | **no** | More files changed is not better work; it is a larger diff. And on `epic_qa` it is **structurally zero** (E41.2's S5, under escalation). |
...
### T5 — **`epic_qa`'s verdict is BLOCKED, and the block is binding**
...
dispatch** under a mutating tool set (`:336-344`), deliberately and permanently.

**The P12 Phase Chat has made the consequence binding on this epic:**

> **E41.3 may not apply an unresolved floor to a candidate.** The Hard Constraint requires the bar
> committed before the run it judges, and **a bar under escalation is not a committed bar.**

**What this epic may do:** plan every `epic_qa` run, execute it, record every raw count, and state
the comparison in raw numbers. **What it may not do:** issue an `epic_qa` pass/fail verdict or a
row recommendation against a floor the CFO has not ruled on. **`epic_dev` is unaffected in both
planning and execution.**

**If S5 returns before this epic's runs, apply the ruled floor and say which version you applied.**
...
- reach one of: **candidate clears**, **no candidate clears — row holds**, or (for `epic_qa`)
  **verdict withheld pending S5**, with the raw comparison recorded either way.
...
- [ ] **D2** — Two per-row applications of the relative bar, each stating both conditions explicitly
      — or the withheld-verdict statement for `epic_qa` (T5)
...
- [ ] The `epic_qa` row states the raw comparison and either applies a **ruled** floor or **records
      the verdict as withheld pending S5** — never applies an unresolved floor
...
- [ ] **Per-row recommendation states the relative bar's two conditions explicitly, or states that no
      candidate cleared** — and, for `epic_qa`, may state the verdict as withheld pending S5 rather
      than applying an unruled floor
...
**Blockers**
- **`epic_qa` verdict only**, pending S5. `epic_dev` has none.
...
flowchart TB
  ...
  QA ==> T5{{"T5 — VERDICT BLOCKED<br/>S5 unresolved: files changed > 0 is<br/>UNREACHABLE on this lane by construction<br/>a bar under escalation is NOT a committed bar<br/><br/>record raw counts · WITHHOLD the verdict"}}
...
- **Description:** E41.3 takes E41.2's instrument, bar and tasks **unmodified** and runs eight
  ...
  **withheld** while S5 is unresolved, because a bar under escalation is not a committed bar (T5).
