...
`<function=…>` as prose — **a lane property, and M42 repairs the lane.** If the repaired path parses
that syntax, **the `FAIL 0/20` mode may simply not recur and the baseline may be stable on its
own.** **The ruled bar costs nothing if so, and was fixed before anyone could see which way it
went.**
