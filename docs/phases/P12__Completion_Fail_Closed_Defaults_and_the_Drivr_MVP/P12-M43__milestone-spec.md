...
| Version | Date | Change |
|---|---|---|
| 1.2.0 | 2026-09-03 | **CLOSURE ACCEPTED by the Phase Chat, and `status` flipped to `completed` in the same act as consolidation** (the M42 lesson: a stale `planned` is load-bearing — a downstream gate read it as "not done"). Acceptance was **named and attributable, per E43.3's sibling E43.2** — the first acceptance in this framework that could not have been performed by silence, binding the acceptance of the milestone that landed the rule. **Verified by re-measurement (G2), not report:** suite **740 passed / 0 failed**; validator **0 errors / 1 warning** (`model_verification` only, both gate keys clean); all four normative landings present in `PROJECT-SYSTEM-GUIDELINES.md` (`:589` parent-merges, `:562` silence-accepts-nothing, `:617-619` the single rework statement, `:632` the flip); **`P12-GH-1` substantively closed** — 7 of 25 templates now reach the rework limit, including all three starter templates, against an original finding of **zero**. **A fourth carry-forward was added at review:** the Drivr half of E43.4 was verified (469 passed) but **unmerged** — measured on `epic/P12-M43-E43.4`, one commit ahead of a `main` that contained none of it. The declaration gave the number without the ref, and the ref was load-bearing. Merged to Drivr `main` under the same authorization as this consolidation. |
