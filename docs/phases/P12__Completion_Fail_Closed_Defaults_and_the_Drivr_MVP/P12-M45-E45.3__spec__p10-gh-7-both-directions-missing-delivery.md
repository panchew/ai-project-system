...
**Baselines:** Drivr **469** at `f15e239`; `ai-project-system` is **environment-dependent** — `766+1 skipped` /
`767 passed` / `766 passed + 1 failed` are the same suite on `milestone/M45`, the variance being the
live-ComfyUI integration test (see the Starter's Suite Baseline). A re-measure that returns any of the
three is not a regression.
