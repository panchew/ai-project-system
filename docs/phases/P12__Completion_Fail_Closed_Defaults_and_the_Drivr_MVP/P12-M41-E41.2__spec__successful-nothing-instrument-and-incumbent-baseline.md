...
### S5 — **RULED. The absolute floor was unsatisfiable for `epic_qa` by construction; it is now per-lane**
...
**The bar's shape is settled above this chat**, so this was **escalated, not decided** — and it has
been **RULED.**
...
> merged to `master` at `e9834c6`; this spec's milestone was amended at **v1.4.0**.
>
> | Lane | Floor |
> |---|---|
...
>
> **The reasoning of record, which is why this is not a relaxation.** `files changed > 0` on
> `epic_qa` was **a constant false, not a strict bar** — a check with the same answer for every
> candidate has **zero discriminating power**. And `files changed` was the **dev lane's proxy for
> "it actually acted"**; on a lane forbidden to write, **acting is reading and grounding**, which
> `tool rounds > 0` and `claims resolve` already measure. **The floor's purpose survives intact.**
>
> **Nothing is lost against the record:** E39.3's fabrication fails `rounds > 0` **and**
> `claims resolve` **independently**.
>
> **⚠ Criterion 2 is untouched and stays untouched.** **Reaching this floor by enabling a mutating
> tool on the QA lane is refused in advance** — `bin/run-qa-agent` refuses to dispatch under a
> mutating tool set, and that refusal is not a side effect to be routed around. **A floor cleared by
> weakening the read-only guarantee is not a cleared floor.**
>
> **Three levels each declined to adopt the per-lane form as an application detail** — this Epic's
> Milestone Chat, the Phase Chat, and HQ — and put it to the CFO as the owner of Binding Constraint
> 6, ratified in SN-36/37. **He ruled it.** The escalation is the reason the floor is now
> discriminating rather than constant.

**`epic_qa`'s verdict is UNBLOCKED.** This epic measures both baselines and **applies the ruled
per-lane floor**, and **states which version of the floor it applied** — the milestone requires that
statement explicitly, because a run judged against the pre-ruling floor and one judged against the
ruled floor are not comparable results.
...
- **Re-open S5.** It is **ruled** — the floor is per-lane by CFO decision. Apply it; do not re-derive it, and do not reach it by weakening Criterion 2.
...
— **per lane, as ruled (S5)**: `epic_dev` at `rounds > 0 AND files changed > 0`, `epic_qa` at
`rounds > 0 AND claims resolve`, with `files changed` **recorded but not scored**. **State which
version of the floor you applied.**
...
- **carriers of at least one DIRECTIONAL check with objective ground truth, committed with the
  task** — see the box below; **this is the term the relative bar attaches to and without it E41.3
...
> ### ⚠ Each task must carry a DIRECTIONAL check, and the instrument's counters are not one
>
> *Added v1.0.2, raised by E41.3's planning (its T3). **This is a requirement on the tasks, not a
> change to the bar.***
>
> The relative bar is *no worse on **every objective check** and **strictly better on at least
> one**, over an **absolute floor**.* **Checks and floor are separate terms** — and **the three
> counters this instrument produces are floors, not scores:**
>
> | Counter | Orders candidates? | Why not |
> |---|---|---|
> | tool rounds | **no** | **More is not better.** E33.2 Run B took **10 rounds**, wasted **3** on a denied `pip install -e .`, and hit `max_iterations_exceeded` with the work essentially done. Four rounds doing the same work is not worse. |
> | files changed | **no** | A larger diff is not better work — and on `epic_qa` it is structurally **zero** (S5). |
> | claims resolving | **only as a RATE** | resolved / asserted is directional and objective; a raw count is not. |
>
> **So "strictly better on at least one" has almost nothing to attach to, and the only remaining
> tie-breaker would be a quality judgment — which is prohibited.**
>
> **Therefore each task carries its own objective ground truth, committed before any run:**
>
> - **`epic_dev`** — **a pre-written test the produced work either passes or does not.** E33.2's
>   target epic is the worked example (`tests/test_public_api.py`, suite 210 passed / 1 skipped).
>   *"The suite passes"* is mechanical, not a quality judgment.
> - **`epic_qa`** — **known ground truth on the assessed artifact**: how many genuine or planted
>   defects the run identifies, and how many things it asserts that are not true. Counting catches
>   and false alarms against ground truth **committed before the run** is objective. **It is not the
>   back-test** — same shape of scoring, different instrument, different task.
>
> **A task without a directional check cannot be compared, and E41.3 escalates rather than
> improvising one after seeing the candidates.** Choosing a scoring rule once you know who is being
> scored is exactly the failure the *bar-committed-first* rule exists to prevent.
...
- [ ] **D3** — The committed objective-check list and **per-lane floor as ruled**, committed
      **before** any measurement run, with **the version of the floor stated**
...
- [ ] **D5** — The dispatch-path statement, with the engine-resolves and Docker-present questions
      answered explicitly
- [ ] **D6** — Escalation Notice(s) where required. **S5 is ruled and needs none** — apply the
      per-lane floor and record which version you applied
...
- [ ] **`epic_qa` is judged against the RULED per-lane floor** — `rounds > 0` AND `claims resolve`,
      with `files changed` **recorded but not scored** — and the record **states which version of the
      floor was applied**
- [ ] **Criterion 2 is intact**: no run reached its floor via a mutating QA tool set, and the record
      says so
- [ ] Two separate incumbent baselines exist, `epic_dev` and `epic_qa`, each with two runs, all
      reported, none discarded silently
- [ ] The task definitions are committed such that E41.3 can run the identical task, **and each
      carries at least one directional check with objective ground truth** — without it E41.3 cannot
      apply the relative bar
...
**External / CFO-side**
- **✔ RESOLVED — S5, the `epic_qa` floor.** Ruled per-lane by the CFO, 2026-08-20. **Nothing in this
  epic is blocked by it**, and E41.3 may now apply the ruled floor to a candidate.
...
flowchart TB
  ...
  S5{{"S5 — RULED: THE FLOOR IS PER-LANE<br/>floor says files changed > 0<br/>run-qa-agent:337 REFUSES a writable<br/>tool set — QA can never clear it<br/>= M40's F5, one level down<br/>bar's shape is above this chat"}}
  S5 ==>|"epic_qa: rounds>0 AND claims resolve<br/>files changed RECORDED, NOT SCORED"| D4
...
- **Description:** E41.2 in execution order, with the two findings that reshaped it. The checks and
  ...
  (S3). The `epic_qa` floor was **ruled per-lane** after this epic raised it: the QA lane is
  read-only by enforcement, so *files changed > 0* was a **constant false** rather than a strict bar,
  and on that lane it is replaced by *claims resolve*, with files-changed recorded but not scored
  (S5). Proposed-track Structural diagram
...
- **The R6 manual-surface rule does NOT reach this epic, and that is explicit rather than inferred.**
  HQ's ruling of 2026-08-20 gates the landing of the five **manual verification targets** on a
  confirmed surface. Its scope statement names `epic_dev` and `epic_qa` as **excluded**: the dispatch
  lanes **land on E41.3's evidence alone.** So nothing in that ruling changes this epic's
  instrument, its replay set, its tasks or its baselines. **S5's `epic_qa` floor question is a
  separate, still-open escalation** and must not be confused with it.
...
- **On S5, now that it is ruled — what the escalation was, and what it was not.** It was not an
  argument that the floor is wrong, and not a proposal to weaken it. **The floor is the CFO's instrument and its adverse results are the point.** What is
  ...
  obvious resolution stated so the escalation is answerable, and not adopted** — and the CFO ruled
  that resolution. **The floor that came back is more discriminating than the one that went up**,
  because a constant-false term was replaced by one that can distinguish candidates. **That is what
...
| Version | Date | Change |
|---|---|---|
| 1.1.0 | 2026-08-20 | **S5 is RULED and `epic_qa`'s verdict is UNBLOCKED.** The CFO ruled the qualification floor **per-lane** (`.ai-project/artifacts/rulings/2026-08-20__ai-project-system-hq__ruling__s5-per-lane-qualification-floor.md`, merged to `master` at `e9834c6`; milestone spec v1.4.0): `epic_dev` keeps `tool rounds > 0` AND `files changed > 0`; **`epic_qa` takes `tool rounds > 0` AND `claims resolve against files that exist`, with `files changed` recorded but not scored.** Raised here as S5, verified independently by the Phase Chat and HQ, and **ruled by the CFO as the owner of Binding Constraint 6** — three levels each declined to adopt the per-lane form as an application detail. The reasoning of record: `files changed > 0` on `epic_qa` was **a constant false, not a strict bar** — zero discriminating power — and `files changed` was the dev lane's proxy for *it actually acted*, whose read-only equivalent is reading and grounding. **Every recorded historical failure still fails**, and **Criterion 2 is untouched: reaching this floor by enabling a mutating QA tool set is refused in advance.** Consequential edits: the S5 finding now carries the ruling's terms; the floor in D3 is per-lane; the DoD requires the epic to **state which version of the floor it applied** and to record that Criterion 2 stayed intact; D6 no longer expects an S5 escalation; the dependency is marked resolved. **A minor bump rather than a patch, because a deliverable that was withheld is now required.** No scope, deliverable-count, ordering or measurement change. |
| 1.0.2 | 2026-08-20 | **Adds a requirement to the task design, raised by E41.3's planning (its T3): each task must carry at least one DIRECTIONAL check with objective ground truth, committed with the task.** The relative bar's *strictly better on at least one* attaches to an **objective check**, and **this instrument's three counters are floors, not scores** — more tool rounds is not better (E33.2 Run B: 10 rounds, 3 wasted, `max_iterations_exceeded`, work done), a larger diff is not better work, and claims-resolution orders candidates **only as a rate**. Without a directional check the only remaining tie-breaker is a quality judgment, **which is prohibited**, and E41.3 would be unable to compare anything. The natural forms are named — a **pre-written test** for `epic_dev` (E33.2's worked example), **known ground truth on the assessed artifact** for `epic_qa`, explicitly not the back-test. **The requirement lands here rather than in E41.3 because a scoring rule chosen after the candidates are known is the exact failure the bar-committed-first rule prevents.** DoD updated. No change to the instrument, the replay set, the floor, or any measurement. |
| 1.0.1 | 2026-08-20 | Records that the **R6 manual-surface rule (HQ Ruling, 2026-08-20) does not reach this epic** — its scope is the five manual verification targets and **explicitly excludes `epic_dev`/`epic_qa`**, which land on E41.3's evidence alone. Added because the milestone spec moved to v1.2.0 while this set was in flight and an executor would otherwise have to infer the exclusion. **S5's `epic_qa` floor escalation is separate and still open.** No change to deliverables, the replay set, the tasks, the floor, or any measurement. |
