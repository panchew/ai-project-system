...
> # ⚠ WHAT THIS EPIC IS NOW — READ THIS INSTEAD OF THE SCOPE BELOW
>
> *v2.0.0, 2026-09-01. The governing statement is **M41 spec v1.6.0's E41.5 Epic Detail**, which the
> Phase Chat wrote; this section applies it. **Everything beneath is preserved for the record and is
> no longer the work.***
>
> ## The line-up landed without this epic. #236 did it, on `master` `3222f50`
>
> By the route SN-40..46 Decision 6 mandated — *Steering Note → HQ ruling → one PR, outside P12's
> milestone machinery* — because these are **governance configuration, not phase work.**
>
> **So this epic lands nothing.** Its original deliverables 1–5 are discharged by another party.
>
> ## Three of this spec's own findings are moot, and all three were this chat's
>
> | Finding | Why it is moot |
> |---|---|
> | **V1** — the `bin/` collision is zero unless a lane moves | **No lane moved.** #236 edited `DEFAULT_MODELS` itself, and a divergence guard caught it **failing loudly** |
> | **V3** — landing `creation` forces the shared-scalar refactor | **`EXPECTED_MANUAL_ONLY_VALUE` is gone.** The test is now `test_config_manual_only_key_is_well_formed`. **Right about the constraint, wrong about who would hit it** |
> | **The notification DoD clause** | It rested on *five verification targets arming simultaneously*. **`model_verification: advisory` means NONE arms.** Narrowed five → three → at-most-one → **zero** |
>
> ## Both gates are MOOT, and saying so is the honest thing
>
> **Gate 1 (M42 closed)** guarded attributability for a landing that has already happened.
> **Gate 2 (every moving row passed its harness)** is **suspended by CFO decision** — no gate blocks a
> line-up change until he declares enforcement, which also suspends HQ's fidelity condition.
>
> **Recording them as moot is honest; leaving them stated as live would be theatre.**
>
> ## What remains — small, real, and the reason this is NARROWED rather than SUPERSEDED
>
> **1. `model-routing-policy.md`'s row P4 and its mapping row no longer describe one thing.**
>
> - **Row P4** still justifies *"Paid frontier"* by **largest spend share**, **Stage-2 accept
>   authority**, and *"evaluated against local inference 2026-07-31 and not moved."*
> - **Its mapping row** now says the value is *"set by CFO allowance decision (SN-41), **not by
>   measurement**."*
>
> **Neither is wrong. Together they are incoherent.** #236 correctly moved the mapping and left the
> tier row alone. **Nobody has yet recorded that the row's justification no longer describes how its
> value is chosen.**
>
> **⚠ And E41.4's result now bears on that sentence directly.** Row P4 cites *"evaluated against local
> inference and not moved."* **There is now a second evaluation** — E41.4, merged 2026-09-01 — and its
> result is **neutral**: `claude-opus-5`, `gpt-5.6-sol` and `deepseek-v4-pro` are **identical on every
> objective check**, so neither candidate cleared the relative bar. **The honest reconciliation is
> therefore available and specific:** the value is set by allowance, and **the one measurement this
> project has neither supports nor contradicts it.**
>
> **2. Record the discharge.** What #236 landed, what this milestone had planned, and **where the two
> differ.** **A milestone whose terminal epic was overtaken should say so in its own record rather
> than leave a reader to infer it from an absence.**
>
> ## Deliverables, replacing D1–D5
>
> - **D-A — Row P4 reconciled.** Its stated reasoning made to describe how its value is actually set,
>   **beside the row** rather than by moving it (row P4's own precedent). **Cite E41.4's neutral
>   result rather than restating it.**
> - **D-B — The discharge record.** Landed vs planned vs differing, with commits.
> - **D-C — Epic Delivery Notice.**
>
> **Not deliverables any more:** the `.ai-project.yml` edit, `DEFAULT_MODELS`, `chat-hierarchy.md`'s
> mapping, the test refactor, the notification. **All done by #236 or made moot.**
>
> ## What has NOT changed
>
> - **The mapping table and row P4's `default` cell still stay put.** The table is forced by
>   `test_policy_mapping_agrees_with_yml_block`; **the cell is held by choice**, because it sits
>   outside the guard's parse and **a divergence no test can catch is worse than one that fails a
>   build.**
> - **The three-row carry-forward** (`phase`, `milestone`, `epic_manual`) — **superseded in substance
>   by the landing**, and its status is the Phase Chat's to state, not this epic's to assume.
> - **Suite green, invocation stated, no skips introduced.**
...
- [ ] **Every guard that binds a key this epic actually changes is green and unweakened — and the
      record CITES THOSE GUARDS, not a count.** For a `creation`-only landing they are the two over
      **`MANUAL_ONLY_KEYS`** (`:247`, `:271`), **not** the two over `MODEL_KEYS` (`:205`, `:222`),
      which never see it (V2)
...
- **E41.3** — the two lane conclusions, **both reached**. S5 is ruled (per-lane floor, CFO,
  2026-08-20), so neither row arrives withheld. **A row where no candidate cleared still holds** —
  that is a reached conclusion, not a missing one.
...
- **✔ RESOLVED — S5.** Ruled per-lane, 2026-08-20. `epic_qa` is decidable; a row still holds if no
  candidate cleared it.
...
| Version | Date | Change |
|---|---|---|
| 2.0.0 | 2026-09-01 | **MAJOR — this epic no longer lands the line-up; #236 did, outside M41's machinery.** Applies M41 spec **v1.6.0**'s E41.5 Epic Detail, which is the governing statement. **Three of this spec's own findings are moot and all three were this chat's:** V1 (no lane moved; #236 edited `DEFAULT_MODELS` and a divergence guard caught it failing loudly), V3 (`EXPECTED_MANUAL_ONLY_VALUE` is gone — right about the constraint, wrong about who would hit it), and the notification clause (advisory-by-default means **nothing arms**; narrowed five → three → at-most-one → **zero**). **Both gates recorded as MOOT rather than left stated as live**, which would be theatre. **What remains is small and real:** row P4's justification no longer describes how its value is set, and **E41.4's neutral result now bears on it directly** — the row cites *"evaluated against local inference and not moved"*, and there is now a second evaluation whose candidates were **identical on every check**, so the honest sentence is that the value was set by allowance and the one measurement neither supports nor contradicts it. **Deliverables replaced by D-A (reconcile row P4 beside the row), D-B (record the discharge) and D-C (Delivery Notice).** Original scope preserved beneath, unmodified. |
| 1.0.2 | 2026-08-20 | **Records that S5 is ruled** (per-lane qualification floor, CFO, 2026-08-20) and removes this spec's two dependencies on its being open. E41.3 now delivers **both** lane conclusions reached rather than one possibly withheld, so this epic no longer has to treat a withheld `epic_qa` verdict as a distinct input state. **A row where no candidate cleared still holds** — that remains a reached conclusion, not a missing one, and nothing about Gate 2 changes. No scope, deliverable, gate or ordering change. |
| 1.0.1 | 2026-08-20 | **Mirrors milestone spec v1.3.0's amended acceptance criterion** into this epic's own, after the Phase Chat folded V1/V2/V3 into F3 as its second correction. The criterion now requires the epic to **cite the guards that bind what landed rather than repeat a count** — for a `creation`-only landing, the two over `MANUAL_ONLY_KEYS`, not the two over `MODEL_KEYS` that never see it. The requirement was already in this spec's Scope, DoD and V2 finding; this puts it in the acceptance criteria too, so the epic is judged against it rather than merely instructed by it. **Milestone spec v1.3.0 (`cd9f01a`) re-read in full before this amendment** — F3's second correction, the promoted V5 block, and the amended acceptance criteria. **No scope, deliverable, gate or ordering change.** |
