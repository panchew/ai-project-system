...
| Version | Date | Change |
|---|---|---|
| 1.2.0 | 2026-09-07 | **CLOSURE ACCEPTED by the Phase Chat, and `status` flipped to `completed` in the same act as closure consolidation** (the M42–M46 precedent). **Verified from raw data, not from the record (G2):** **C-A = 111** parsed directly from `transcript-gemini.json` (111 `tool_call`/`tool_result` entries), **C-B = 21** derived independently from the two commits on `panchew-io` `epic/E1.1` (17 + 4, deduplicated), instrument verdict **`PASS`** at `worktree` provenance with `lower_bound: false`. **The exit code is structurally excluded from the verdict** — the instrument scores only terms marked `scored`, and `exit_code` sits in `recorded_never_scored`. **Finding 1 elevated to a phase carry-forward because it is LIVE:** `panchew-io` has **no `models:` block** and the CFO-authorized fix `863eb49` is **orphaned** from both `main` and `milestone/M1` — a subject project reverted an authorized configuration through its own restructure, surfacing only in a retrospective note. The proof succeeded because the run was routed around it, so **the absence produced no signal**. M47 is P12's last milestone; on this consolidation the phase moves to closure under PSG §5C. |
