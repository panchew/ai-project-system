...
falsification demonstration (reintroduce a duplicate → check fails → restore → green). **The fence
detector the check relies on must handle the non-triple/indented markers described in D1, and the check
must fail loudly on an odd fence-line count** — a parity mismatch is the detector desyncing, and silent
wrong output is the failure mode this check exists to prevent.
